...
new_doc = Document()
document = Document(r"C:\Users\nysha\OneDrive\Documents\segments_testing.docx")
...
for block in iter_block_items(document):
    if 'text' in str(block):
        ...
        # new_doc.add_paragraph(appendtxt)
        isappend = True
        for run in block.runs:
            ...
            if 'pic:pic' in xmlstr:
                ...
                for pic in root.findall('.//pic:pic', my_namespaces):
                    ...
                    style = 'Novalue'
                    # new_doc.add_picture(image_part)
...
#     """
#     Parse XML string and return ElementTree root.
#     """
...
#     # If the content is a paragraph
#     if row['table_id'] == 'Novalue':
#         new_paragraph = new_doc.add_paragraph(row['para_text'])
#         # Apply paragraph style if available
#         if row['style'] != 'Novalue':
#             new_paragraph.style = row['style']
#     # If the content is a table
#     else:
#         # Retrieve the table data from table_mod using the table_id
#         table_data = table_mod[table_mod['table_id'] == row['table_id']].iloc[0]['string_value']
#         # Parse the table data into a table element
#         table_element = parse_xml(table_data)
#         # Add the table element to the new document
#         new_doc._body.append(table_element)
#     # If the content is an image
#     if 'Document_Imagefile' in row['para_text']:
#         # Retrieve image data from image_df using the image_index
#         image_data = image_df[image_df['image_index'] == int(row['para_text'].split('/')[-1])].iloc[0]
#         # Add the image to the new document
#         new_doc.add_picture(image_data['image_base64_string'])
